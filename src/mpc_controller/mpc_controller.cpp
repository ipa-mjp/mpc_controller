...
bool MpcController::initialize(void)
{
    ...
    kdl_tree.getChain( controller_param_.chain_base_link, controller_param_.chain_tip_link, controller_param_.chain_ );
    if (controller_param_.chain_.getNrOfJoints() == 0)
    {
    ...
    for (uint16_t i = 0; i < controller_param_.chain_.getNrOfSegments(); ++i)
    {
    	controller_param_.frame_names.push_back(controller_param_.chain_.getSegment(i).getName());
    }
	...
	this->joint_states_.current_q_dot_ 	= KDL::JntArray( controller_param_.chain_.getNrOfJoints() );
	this->joint_states_.last_q_ 		= KDL::JntArray( controller_param_.chain_.getNrOfJoints() );
	this->joint_states_.last_q_dot_ 	= KDL::JntArray( controller_param_.chain_.getNrOfJoints() );
...
void MpcController::jointStateCallBack(const sensor_msgs::JointState::ConstPtr&  msg)
{
...
void MpcController::unitTestFunctionlity(void)
{
